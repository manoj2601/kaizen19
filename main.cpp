...
map <int, vector <int> > mass;
   mass[1]=v1;  mass[2]=v2; mass[3]=v3; mass[4]=v4; mass[5]=v5; // for coordinates
   map <int, vector <int> > masst;
   masst[1]=v1t;  mass[2]=v2t; mass[3]=v3t; mass[4]=v4t; mass[5]=v5t; // for time
// data complete


// finding right place
 vector <int> vel; int rt=1;

     pair <string, string> prob; for(prob:hos_op)
{ string word;
...
vector <int >  disto;
for (int o = 0; o < vel.size (); o++ )
{
    int u = vel[o] ;
    vector <int> happy =mass[u];
    int distance_x = v[0]-happy[0];
    int distance_y = v[1]-happy[1];
  ...
  for (int l=0 ; l<vel.size(); l++)
  {int f = vel[l]; string sol =name[f]; int d = disto[f];
      cout << sol << " " << d << " "; masst[l]; vector <int> vello;
      for (int i =0 ; i< 2; i++)
      {
          if (vello[0]== 0 && vello[1]== 0){ cout << " always there in your service"<< endl;}
          else { cout << "  opening time  "<< vello[0]<< " closing time " << vello[1]<< endl;
      }
      }
  }
